...
public class Shooter extends SubsystemBase implements Logged, AutoCloseable {
  @Log.File
  private final FlywheelIO flywheel = Robot.isReal() ? new RealFlywheel() : new SimFlywheel();

  @Log.NT private final PIDController pidController = new PIDController(kp, ki, kd);
  ...
  public boolean isAtShootingSpeed() {
    return pidController.getSetpoint() == CONSTANT_TARGET_RPS && isAtGoal();
  }

  public Shooter() {
    setDefaultCommand(runOnce(() -> pidController.setSetpoint(0.001)).andThen(Commands.idle(this)));
  }
  ...
  public Command shoot() {
    return run(() -> pidController.setSetpoint(CONSTANT_TARGET_RPS));
  }
